...
/**
 * @brief True if any up interface holds a routable IPv4 address.
 */
static bool have_ipv4_address(void)
{
...
static int do_iperf_cmd(int argc, char **argv)
{
    ...
    if (!have_ipv4_address()) {
        ESP_LOGE(__func__, "No interface has an IPv4 address yet");
        return 1;
    }
