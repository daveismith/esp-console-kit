/**
 * From examples/protocols/icmp_echo/main/echo_example_main.c
 */
...
static EventGroupHandle_t ping_events = NULL;

#define PING_RUNNING (1 << 0)
#define PING_COMPLETE (1 << 1)
...
static void cmd_ping_on_ping_end(esp_ping_handle_t hdl, void *args)
{
    ...
    uint32_t loss = transmitted ? (uint32_t)((1 - ((float)received) / transmitted) * 100) : 100;
    ...
    // Notify the system that the ping operation is complete
    xEventGroupSetBits(ping_events, PING_COMPLETE);
}
...
static int do_ping_cmd(int argc, char **argv)
{
    ...
    EventBits_t bits = xEventGroupGetBits(ping_events);
    ...
    if ((bits & PING_RUNNING) != 0) {
        printf("ping: ping in progress\n");
        return 1;
    }
    xEventGroupSetBits(ping_events, PING_RUNNING);
    ...
    if (inet_pton(AF_INET6, ping_args.host->sval[0], &sock_addr6.sin6_addr) == 1) {
        ...
        if (getaddrinfo(ping_args.host->sval[0], NULL, &hint, &res) != 0) {
            ...
            xEventGroupClearBits(ping_events, PING_RUNNING);
            return 1;
        }
    ...
    printf("PING %s (%s): %"PRIu32" data bytes\n", ping_args.host->sval[0], ipaddr_ntoa(&target_addr), config.data_size);
    err = esp_ping_start(ping);
    if (err != ESP_OK) {
        printf("ping: cannot start session: %s\n", esp_err_to_name(err));
    ...
    xEventGroupWaitBits(ping_events, PING_RUNNING | PING_COMPLETE, pdTRUE, pdTRUE, portMAX_DELAY);
    return 0;
}
