...
static void iperf_report_task(void *arg)
{
    ...
    while (!s_iperf_ctrl.finish) {
        ...
        printf("%4"PRIu32"-%4"PRIu32" sec       %.2f Mbits/sec\n", cur, cur + interval,
            actual_bandwidth);
        cur += interval;
        ...
        if (cur >= time) {
            printf("%4d-%4"PRIu32" sec       %.2f Mbits/sec\n", 0, time,
                average);
            break;
        }
    }
...
static esp_err_t iperf_run_udp_client(void)
{
    ...
    if (s_iperf_ctrl.cfg.type == IPERF_IP_TYPE_IPV6) {
        ...
        ESP_GOTO_ON_FALSE((client_socket >= 0), ESP_FAIL, exit, TAG, "Unable to create socket: errno %d", errno);
        ESP_LOGI(TAG, "Socket created, sending to %"PRIu32":%d", s_iperf_ctrl.cfg.destination_ip4, s_iperf_ctrl.cfg.dport);
