...
static esp_err_t session_begin(session_t *s, uint32_t baud)
{
    const int port = s_cfg.uart_num >= 0 ? s_cfg.uart_num : CONSOLE_UART;
    if (port < 0 || !uart_is_driver_installed((uart_port_t)port)) {
        printf("fs: no UART driver on the console; XMODEM needs one\n");
    ...
    if (s->baud != s->console_baud) {
        ...
        if (err != ESP_OK) {
            ...
            printf("fs: cannot run the UART at %" PRIu32 " baud\n", s->baud);
            return err;
        }
    }
    return ESP_OK;
...
static const char *xmodem_err(esp_err_t err)
{
    switch (err) {
...
static int fs_put(int argc, char **argv)
{
    ...
    const int port = s_cfg.uart_num >= 0 ? s_cfg.uart_num : CONSOLE_UART;
    if (port >= 0) {
        uart_get_baudrate((uart_port_t)port, &console_baud);
    }
    char size_text[16];
    ...
    esp_err_t err = session_begin(&session, baud);
    ...
    if (err == ESP_OK) {
        ...
        printf("fs: put failed: %s, after %u bytes (%u retries: %u timed out, %u bad)\n",
               xmodem_err(err), (unsigned)stats.bytes, stats.retries, stats.timeouts,
               stats.bad_blocks);
